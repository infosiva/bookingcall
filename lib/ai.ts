...
// Qwen via OpenRouter — free tier: 70M tokens/month
// Sign up: openrouter.ai → free key, no card needed
// Models ending in :free have no cost on free tier
const DEFAULT_QWEN_TIERS: Record<Quality, string[]> = {
  fast:     ['qwen/qwen3-8b:free'],
  balanced: ['qwen/qwen3-30b-a3b:free', 'qwen/qwen3-8b:free'],
  best:     ['qwen/qwen3-235b-a22b:free', 'qwen/qwen3-30b-a3b:free'],
}
...
// ── Core callAI ───────────────────────────────────────────────────────────────
// Order: Ollama → Groq → Gemini → Cerebras → Qwen (OpenRouter free 70M/mo) → OpenAI → Anthropic
export async function callAI(
  ...
  const providers = [
    ...
    { name: 'qwen',      fn: () => callProvider('https://openrouter.ai/api/v1',                            'OpenRouter',  'OPENROUTER',  tiers.qwen[quality],     system, messages, maxTokens) },
